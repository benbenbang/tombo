...
const RE_REQUIREMENT = /^([A-Za-z0-9_\-.]+)(?:\[([^\]]+)\])?(?:\s*([<>=~!]+)\s*([0-9a-zA-Z.*+-]+))?/;

// Export the regex for use in quickFill.ts
export const RE_REQUIREMENT_VERSION = /^([A-Za-z0-9_\-.]+(?:\[[^\]]+\])?)\s*([<>=~!^]+)\s*(.*)$/;

/**
 * Parse requirements.txt content into Package objects
 * @param text The content of requirements.txt
 * @returns An array of Package objects
 */
export function parseRequirementsTxt(text: string): Package[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('git+') || line.includes('http://') || line.includes('https://')) {
        ...
        const match = line.match(RE_REQUIREMENT);
        if (match) {
            ...
            pkg.key = packageName;
            pkg.value = version;
            pkg.start = lineStartPosition;
            pkg.end = lineStartPosition + line.length;
...
/**
 * Extracts the package name from a full requirement string
 * @param requirement Full requirement string (e.g., "django>=3.0.0")
 * @returns The package name
 */
export function getPackageName(requirement: string): string {
    const match = requirement.match(RE_REQUIREMENT);
    if (match) {
        return match[1];
    }
    return requirement;
...
/**
 * Checks if a version string is a valid Python package version
...
export function isValidVersion(version: string): boolean {
    ...
    // Simple version validation - can be expanded for more precise validation
